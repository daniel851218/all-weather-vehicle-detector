...
from detector.adv_faster_rcnn import Adversarial_Faster_RCNN
...
def train_one_epoch(model, optimizer, data_loader, writer, epoch, iter_per_batch, log_loss, log_freq):
    ...
    for imgs, targets in pbar:
        ...
        str_loss_daytime = Fore.BLUE + " loss_daytime: " + Fore.RESET + f"{(loss_dict['loss_daytime_img_score'] + loss_dict['loss_daytime_ins_score'] + loss_dict['loss_daytime_consistency']):2.3f}"
        str_loss_weather = Fore.BLUE + " loss_weather: " + Fore.RESET + f"{(loss_dict['loss_weather_img_score'] + loss_dict['loss_weather_ins_score'] + loss_dict['loss_weather_consistency']):2.3f}"
        pbar.set_description(str_loss_total + str_loss_score + str_loss_rpn_box_reg + str_loss_classifier + str_loss_box_reg + str_loss_daytime + str_loss_weather)
        ...
        if iters % log_freq == 0:
            ...
            avg_loss_daytime = sum(log_loss["loss_daytime_img_score"]) / len(log_loss["loss_daytime_img_score"]) + \
                               sum(log_loss["loss_daytime_ins_score"]) / len(log_loss["loss_daytime_ins_score"]) + \
                               sum(log_loss["loss_daytime_consistency"]) / len(log_loss["loss_daytime_consistency"])
            avg_loss_weather = sum(log_loss["loss_weather_img_score"]) / len(log_loss["loss_weather_img_score"]) + \
                               sum(log_loss["loss_weather_ins_score"]) / len(log_loss["loss_weather_ins_score"]) + \
                               sum(log_loss["loss_weather_consistency"]) / len(log_loss["loss_weather_consistency"])
...
if __name__ == "__main__":
    ...
    obj_detector = Adversarial_Faster_RCNN().to(cfg.device)
    if cfg.is_pre_train:
